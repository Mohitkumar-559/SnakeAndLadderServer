...
export function validateNewPosition(playerPos: number, pawnPos: number, diceValue: number, hasKilled?: boolean): any {
  ...
  let newPawnIndex;
  newPawnIndex = diceValue + currentPawnIndex;
  const newPawsPos = pathValue(playerPos, newPawnIndex, hasKilled);
  //console.log("Ivalid ", !!newPawsPos);
  return !!newPawsPos;
}
export function validateNewPositionPower(playerPos: number, pawnPos: number, diceValue: number, hasKilled?: boolean, hasPower?: boolean): any {
  //console.log("validateNewPosition : has killed before ", hasKilled);
  //console.log("\n validateNewPosition \n Player index ", playerPos);
  //console.log("pawn index ", pawnPos);
  //console.log("dice  index ", diceValue);
...
export function isValidPawnPositionPower(playerPos: number, diceValue: number, pawnPos: number, hasKilled?: boolean): boolean {
  if (!pawnPos && diceValue != 6) {
    return false;
  }
  return !!validateNewPositionPower(playerPos, pawnPos, diceValue, hasKilled);
}
export const getRouteFirstValue = (pos: number) => PLAYER_PATH[pos][0];

export function getRange(intial:number,final:number) {
  return PLAYER_PATH[0].slice(intial, final);;
}