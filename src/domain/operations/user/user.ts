...
export class User {
    ...
    private async onGameSync(body: any, callback: any) {
        ...
        if (!game) {
            ...
            if (game.isFinished()) {
                const data = await game.onGameSync(this.userId);
                resp = new BaseHttpResponse(data, null, 200, this.game?.ID);
                TableManager.deleteTableFromMap(game.ID)
            } else {
                const data = await game.onGameSync(this.userId);
                //console.log("Current turn is of "+data.players[data.turnIndex].name)
                resp = new BaseHttpResponse(data, null, 200, this.game?.ID);
    ...
    private async onJoinGame(body: any, callback: any) {
        ...
        // }
            ...
            let data: any = await this.game.onGameSync(this.userId);
            data['syncAfter'] = 5000;
    ...
    public async onMovePawn(body: any, callback: any) {
        try {
            ...
            const diceValue = body.diceValue||0;
            if (!this.game) {
                return;
            }
            const moveResponse = await this.game.onMovePawn(this.userId, pawnIndex, diceValueIndex,diceValue);
    ...
    public async onPowerCard(body: any, callback: any) {
        try {
            ...
            const pawnIndex = Number(Object.values(body)[0]) ;
            const playerID = Object.values(body)[2].toString();
            const powerIndex = Number(Object.values(body)[1]);
            if (!this.game) {
                return;
            }
            const powerCardResponse = await this.game.onPowerCard(this.userId, pawnIndex, powerIndex,playerID);
