...
export class TransactionMethod {
    ...
    async JoinContest(request: TransactionTokenRequest) {
        ...
        try {
            ...
            const proc_name = "PROC_DEDUCT_JOIN_SL_FEES";
            ...
            if (result.length > 0) {
                ...
                const game_proc_name = "PROC_SL_CreateRoomAndAssignToUser";
                let gameParam = "@ContestId=" + request.cid + ", @GameServerId='" + request.gameserverid + "'" ;
                gameParam = gameParam + ", @dtLudoRoomParticipantsJson='" + JSON.stringify(dtCasualRoomParticipants) + "'";
    ...
    async getContestWinners(request: ContestWinnerRequest, gameId: string) {
        try {
            ...
            if (request.ludoParticipantScore.length > 0) {
                ...
                const proc_name = "PROC_DECLARE_SL_GAME_WINNERS_FOR_SOCKET";
